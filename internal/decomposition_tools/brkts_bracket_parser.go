...
func BrktsBracketMatchesPickOut(mccontent MatchTable, game string) ([]liquiwrap.CompletedMatch, error) {
	...
	go func() {
		for i := 0; i < len(mccontent.TokenContent); i++ {
			if GetElClass(mccontent.TokenContent[i].Attr) == "brkts-bracket" {
				globalFind = true
			}
			...
			if globalFind {
				...
				if mccontent.TokenContent[i].Type == html.StartTagToken {
					...
					if mccontent.TokenContent[i].Data == "span" {
						if MatchElClassByRegExp("timer-object", mccontent.TokenContent[i].Attr) && GetElAttribute("data-timestamp", mccontent.TokenContent[i].Attr) != "" {
							dateSeek = true
						}
					}
					if mccontent.TokenContent[i].Data == "div" {
						...
						if MatchElClassByRegExp("brkts-opponent-score-inner", mccontent.TokenContent[i].Attr) {
							scoreSeek = true
							scoreIndex++
						}
						...
						if GetElClass(mccontent.TokenContent[i].Attr) == "block-team" {
							opponentSeek = true
							opponentIndex++
						}

						if MatchElClassByRegExp("brkts-match brkts-match-popup-wrapper", mccontent.TokenContent[i].Attr) {
							bansGameSeek = false
							opponentSeek = false
							scoreSeek = false
							picksGameSeek = false
